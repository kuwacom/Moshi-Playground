...
| 次の発話まで余裕がある                                 | しっかり返答                            |
| 発話間の無音で右ch比率が足りない                       | 無音に短い応答や質問を追加              |
| 無音がほぼなく右ch比率が足りない                       | 任意で文の切れ目に会話用の間を挿入      |
...
既に `datasets/cache/${LORA_NAME}/<音声名>/responses.json` がある場合はキャッシュが優先されます。ただし、現在のモードと違う種類のキャッシュ、左chに被る配置、右ch同士が近すぎる配置は再利用しません。モードを変えて全体を作り直したい時は `--refresh-responses` を付けてください。

### 左右の発話量バランス

Moshi の学習データでは、左chだけが長く続くより、右chもある程度しゃべっている方が安定しやすいです。このため既定では、`--interaction-mode auto` の時に、右chの発話量が足りなければ発話間の無音へ短い応答や質問を追加します。
...
既定では `--target-right-ratio 0.4` を目標にし、`--max-right-ratio 0.5` を超えない範囲で補完します。つまり、左ch : 右ch がだいたい `60:40` 付近になるように、発話間の無音へ文脈に合う短い応答や質問をLLMで生成して足します。元音声は削らず、左chの時間軸も動かしません。左chの発話中に短い相槌を重ねたい場合だけ、`--allow-left-overlap-fill` を明示してください。

無音がほとんどなく比率補完が難しい一人配信では、`--semantic-gap-insert-mode auto` を付けると、LLM が Whisper の元セグメント境界から「ここで質問や応答を挟むと自然」な地点を選びます。その地点以降の左chを後ろへずらして会話用の間を作り、右chのTTSを差し込みます。強制的に一定間隔で割り込むのではなく、直前と直後の発話をLLMへ見せて、話の流れを壊しにくい位置と発話内容を選ばせます。
...
| `--timing-jitter-sec`          |            `0.18` | 応答開始タイミングに足す自然な揺らぎの最大秒数                                    |
| `--response-placement-retries` |               `1` | TTS後に安全な無音へ収まらない場合の短縮再生成回数                                 |
...
| `--long-turn-fill-sec`         |            `12.0` | `--allow-left-overlap-fill` 時に、この秒数以上の左ch発話を途中補完の対象にする    |
| `--fill-interval-sec`          |             `6.0` | 長い無音や明示許可した左ch中で補完候補を置く間隔                                  |
| `--fill-max-chars`             |              `14` | 比率補完で挿入する短い発話の最大文字数                                            |
| `--allow-left-overlap-fill`    |              なし | 比率補完の短い相槌を左ch発話中にも置く                                            |
| `--semantic-gap-insert-mode`   |             `off` | `auto` で比率不足時だけ、`always` で上限まで、文の切れ目に会話用の間を挿入        |
| `--semantic-gap-max-count`     |               `2` | 1音声内で作る会話用の間の最大数                                                   |
| `--semantic-gap-min-turn-sec`  |             `8.0` | この秒数以上の左ch発話まとまりを、会話用の間の候補にする                          |
| `--semantic-gap-max-chars`     |              `36` | 会話用の間に入れる質問・応答の最大文字数                                          |
